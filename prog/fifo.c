...
I<libprog(fifo)> - fifo and file control module
...
doesn't exist or doesn't refer to a fifo. Returns 1 if C<path> does refer to
a fifo. If C<prepare> is non-zero, and C<path> refers to a non-fifo, it will
be unlinked. Returns -1 on error with C<errno> set by I<stat(2)>.
...
another process. Returns 0 if C<path> does not exist or does not refer to a
fifo or if the fifo can't be opened for non-blocking I<write(2)>. If
C<prepare> is non-zero, and path refers to a non-fifo, it will be unlinked.
Returns -1 on error with C<errno> set by I<stat(2)> or I<open(2)>.
...
way. If C<lock> is non-zero, the fifo is exclusively locked. Returns -1 on
error with C<errno> set by I<stat(2)>, I<open(2)>, I<mkfifo(2)>,
