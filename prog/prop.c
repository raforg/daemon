...
I<libprog(prop)> - program properties file module
...
Program names are converted into file name suffixes by replacing every
occurrance of the file path separator (``/'') with a ``-''. Properties files
consists of one property per line. Each property is specified by its name,
followed by C<``=''> followed by its value. The name must not have a
C<``=''> in it unless it is quoted with a C<``\''>. The properties files may
also contain blank lines and comments (C<``#''> until the end of the line).
...
Creates and returns a I<Prop> containing C<map> and C<defaults>. Returns
C<NULL> on error. The new I<Prop> will destroy C<map> and C<defaults> when
it is destroyed.
...
C<char *quote(const char *src, const char *quoted, const char *unquoted, int ascii)>

Returns a copy of C<src> with every occurrance in C<src> of characters in
C<unquoted> replaced with a ``\'' followed by the corresponding (by
position) character in C<quoted>. If C<ascii> is non-zero, other
...
static char *quote(const char *src, const char *quoted, const char *unquoted, int ascii)
{
	...
	length = strlen(src) * 2 + 1;

	if (!(dst = mem_create(length, char)))
		return NULL;

	for (s = src, d = dst; d - dst < length - 2 && *s; ++s)
	{
		char *target = strchr(unquoted, *s);
		...
		if (target)
		{
			...
			{
				*d++ = '\\';
				*d++ = quoted[target - unquoted];
			}
		}
		else if (ascii && !isprint((int)*s))
		{
			if (d - dst < length - 5)
			{
				*d++ = '\\';
...
C<char *unquote(const char *src, const char *quoted, const char *unquoted, int ascii)>

Returns a copy of C<src> with every occurrance in C<src> of ``\'' followed
by a character in C<quoted> replaced with the corresponding (by position)
character in C<unquoted>. If C<ascii> is non-zero, every occurrance in
C<src> of an ASCII code in octal or hexadecimal (i.e. "\ooo" or "\xhh") is
replaced with the corresponding ASCII character. The caller must deallocate
the memory returned.

*/

static char *unquote(const char *src, const char *quoted, const char *unquoted, int ascii)
{
	char *dst;
	const char *start;
	char *slosh;
	...
	for (*dst = '\0', slosh = strchr(start = src, '\\'); slosh; slosh = strchr(start = slosh + 1, '\\'))
	{
		if (ascii)
		{
			int digits = 0;
			...
			if (isodigit((int)*s))
			{
				...
				while (digits < 3 && isodigit((int)s[1]));
			}
			...
			{
				len = strlen(dst);
				snprintf(dst + len, length - len, "%*.*s%c", slosh - start, slosh - start, start, c);
				slosh = s; /* Skip over ASCII code */
				continue;
			}
		}

		if (!(target = strchr(quoted, slosh[1])))
		{
			len = strlen(dst);
			snprintf(dst + len, length - len, "%*.*s\\", slosh - start, slosh - start, start);
			continue;
		}

		len = strlen(dst);
		decode = unquoted[target - quoted];
		snprintf(dst + len, length - len, "%*.*s%c", slosh - start, slosh - start, start, decode);
		++slosh; /* Skip over quoted char */
	}

	len = strlen(dst);
	snprintf(dst + len, length - len, "%s", start);
...
C<char *quote_special(const char *src)>
...
static char *quote_special(const char *src)
{
	return quote(src, special_code, special_char, 1);
}

/*

C<char *unquote_special(const char *src)>
...
static char *unquote_special(const char *src)
{
	return unquote(src, special_code, special_char, 1);
}

/*

C<char *quote_equals(const char *src)>
...
static char *quote_equals(const char *src)
{
	return quote(src, eq, eq, 0);
}
...
static char *unquote_equals(const char *src)
{
	return unquote(src, eq, eq, 0);
}
...
static void prop_parse(Map *map, const char *path, char *line, size_t lineno)
{
	...
	for (eq = strchr(prop, '='); eq; eq = strchr(eq + 1, '='))
		if (eq != prop && eq[-1] != '\\')
			break;
	...
	value = eq + 1;
	conf_skip_spaces_backwards(&eq, prop);
	*eq = '\0';

	/* Unquote any quoted '=' in the name */

	if (!(name = unquote_equals(prop)))
	{
		error("prop: Out of memory");
		mem_release(prop);
		return;
...
obtained from the properties file named C<path>. Returns C<NULL> on error.
...
static Prop *prop_load(const char *path, Prop *defaults)
{
	...
	if (!(map = map_create((list_destroy_t *)free)))
		return NULL;
...
const char *prop_get(const char *name)
{
	...
	for (p = g.prop; p; p = p->defaults)
		if ((value = map_lookup(p->map, name)))
			return value;
...
Sets the property named C<name> to a copy of C<value>. Returns the copy of
C<value> on success, or C<NULL> on error. If I<prop_save()> is called after
a call to this function, the new property will be saved to disk and will be
available the next time this program is executed.
...
Sets the property named C<name> to C<value>. Returns C<value>, or C<0> on
error. If I<prop_save()> is called after a call to this function, the new
property will be saved to disk and will be available the next time this
program is executed.
...
Sets the property named C<name> to C<value>. Returns C<value>, or C<0.0> on
error. If I<prop_save()> is called after a call to this function, the new
property will be saved to disk and will be available the next time this
program is executed.
...
The F<.properties> directory is created if necessary. Returns 0 on success,
or -1 on error.
...
int prop_save(void)
{
	...
	while (list_has_next(k))
	{
		const char *key = (const char *)list_next(k);
		const char *value = map_lookup(g.prop->map, key);
		char *lhs, *rhs, *lhs2;
