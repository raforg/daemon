...
Returns the new I<Msg> object on success, or C<NULL> on error.
...
sends messages to file descriptor C<fd>. Returns the data on success, or
C<NULL> on error.
...
static MsgFDData *msg_fddata_create(int fd)
{
	...
	if (!(data = mem_create(1, MsgFDData)))
		return NULL;
...
Creates a I<Msg> object that sends messages to standard error. Returns the
new I<Msg> object on success, or C<NULL> on error.
...
Creates a I<Msg> object that sends messages to standard output. Returns the
new I<Msg> object on success, or C<NULL> on error.
...
the C<O_WRONLY>, C<O_CREAT> and C<O_APPEND> flags. Returns 0 on success, or
-1 on error.
...
the file specified by C<path>. Returns the data on success, orC<NULL> on
error.
...
By default, it is C<"%Y%m%d %H:%M:%S">. Returns the previous format string
if successful, or C<NULL> on error (if C<format> is C<NULL>).
...
C<path>. Returns the new I<Msg> object on success, or C<NULL> on error.
...
C<facility> is stored to be used when sending messages. Returns 0 on
...
connection to I<syslog>. Returns the data on success, or C<NULL> on error.
...
C<ident>, C<option> and C<facility>. Returns the new I<Msg> object on
...
messages to several I<Msg> objects. Returns 0 on success, or -1 on error.
...
Adds C<msg> to a list of multiplexed I<Msg> objects. Returns 0 on success,
or -1 on error.
...
the list with I<msg_plexdata_add(Msg *msg)>. Returns the data on success, or
C<NULL> on error.
...
*msg)>. Returns the new I<Msg> object on success, or C<NULL> on error.
...
Adds C<item> to the list of I<Msg> objects multiplexed by C<msg>. Returns 0
