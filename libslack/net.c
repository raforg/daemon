...
=item C<int net_server(const char *interface, const char *service, port_t port, int rcvbufsz, int sndbufsz, sockaddr_t *addr, size_t *addrsize)>
...
If C<service> is non-C<NULL> and maps to a port number (as determined by
C<getservbyname(3)>), the specified port is used. Otherwise, C<port> (which
must be in host byte order) is used.
...
use I<getsockopt(2)> to obtain the actual recieve buffer size.
...
If C<addr> is not C<NULL>, the address bound to is stored at that address.
If C<addrsize> is not C<NULL>, the length of C<addr> is stored at that
address. On success, returns the new socket's file descriptor.

On error, returns -1 with C<errno> set appropriately.
...
int net_server(const char *interface, const char *service, port_t port, int rcvbufsz, int sndbufsz, sockaddr_t *addr, size_t *addrsize)
{
	return net_create_server(interface, service, port, SOCK_STREAM, 0, rcvbufsz, sndbufsz, addr, addrsize);
}

/*

=item C<int net_client(const char *host, const char *service, port_t port, long timeout, int rcvbufsz, int sndbufsz, sockaddr_t *addr, size_t *addrsize, size_t *addrsize)>
...
C<service> (as determined by C<getservbyname(3)>) or by C<port> (which must
be in host byte order) otherwise.
...
If C<addr> is not C<NULL>, the address of the peer is stored at that
address. If C<addrsize> is not C<NULL>, the length of C<addr> is stored at
that address.
...
int net_client(const char *host, const char *service, port_t port, long timeout, int rcvbufsz, int sndbufsz, sockaddr_t *addr, size_t *addrsize)
{
	return net_create_client(host, service, port, SOCK_STREAM, 0, timeout, rcvbufsz, sndbufsz, addr, addrsize);
}

/*

=item C<int net_udp_server(const char *interface, const char *service, port_t port, int rcvbufsz, int sndbufsz, sockaddr_t *addr, size_t *addrsize)>

Creates a udp server socket ready to I<recv(2)> packets on C<interface> (as
determined by I<gethostbyname(3)>).
...
int net_udp_server(const char *interface, const char *service, port_t port, int rcvbufsz, int sndbufsz, sockaddr_t *addr, size_t *addrsize)
{
	return net_create_server(interface, service, port, SOCK_DGRAM, 0, rcvbufsz, sndbufsz, addr, addrsize);
}

/*

=item C<int net_udp_client(const char *host, const char *service, port_t port, int rcvbufsz, int sndbufsz, sockaddr_t *addr, size_t *addrsize)>

Creates a udp client socket and connects to the server listening at C<host>
(as determined by I<gethostbyname(3)>) on the port number specified by
C<service> (as determined by C<getservbyname(3)>) or by C<port> (which must
be in host byte order) otherwise.
...
int net_udp_client(const char *host, const char *service, port_t port, int rcvbufsz, int sndbufsz, sockaddr_t *addr, size_t *addrsize)
{
	return net_create_client(host, service, port, SOCK_DGRAM, 0, 0, rcvbufsz, sndbufsz, addr, addrsize);
}

/*

=item C<int net_create_server(const char *interface, const char *service, port_t port, int type, int protocol, int rcvbufsz, int sndbufsz, sockaddr_t *addr, size_t *addrsize)>

Creates a server socket of the specified C<type> (e.g. C<SOCK_STREAM> or
C<SOCK_DGRAM>) and C<protocol> (usually zero) ready to I<accept(2)>
connections on C<interface> (as determined by I<gethostbyname(3)>).
...
static sockaddr_t *net_ipaddr(sockaddr_in *ip, size_t family, void *addr, size_t addrsize, port_t port)
{
	memset(ip, '\0', sizeof(sockaddr_in));
	ip->sin_family = family;
	memcpy(&ip->sin_addr, addr, addrsize);
	ip->sin_port = port;
	return (sockaddr_t *)ip;
}

#ifdef PF_INET6
static sockaddr_t *net_ip6addr(sockaddr_in6 *ip, size_t family, void *addr, size_t addrsize, port_t port)
{
...
int net_create_server(const char *interface, const char *service, port_t port, int type, int protocol, int rcvbufsz, int sndbufsz, sockaddr_t *addr, size_t *addrsize)
{
	...
	if (service && (protoent = getprotobynumber(protocol)) && (servent = getservbyname(service, protoent->p_name)))
		port = servent->s_port;
	else
		port = htons(port);
	...
	if (interface)
	{
		...
		if (hostent->h_addrtype == PF_INET && hostent->h_length == sizeof ip.v4.sin_addr)
		{
			ipaddr = net_ipaddr(&ip.v4, hostent->h_addrtype, hostent->h_addr, hostent->h_length, port);
			ipsize = sizeof ip.v4;
		}
#ifdef PF_INET6
		else if (hostent->h_addrtype == PF_INET6 && hostent->h_length == sizeof ip.v6.sin6_addr)
		{
			ipaddr = net_ip6addr(&ip.v6, hostent->h_addrtype, hostent->h_addr, hostent->h_length, port);
			ipsize = sizeof ip.v6;
		}
#endif
		else
	...
	if (rcvbufsz && setsockopt(sockfd, SOL_SOCKET, SO_RCVBUF, &rcvbufsz, sizeof(rcvbufsz)) == -1)
		return close(sockfd), -1;

	if (sndbufsz && setsockopt(sockfd, SOL_SOCKET, SO_SNDBUF, &sndbufsz, sizeof(sndbufsz)) == -1)
		return close(sockfd), -1;
...
=item C<int net_create_client(const char *host, const char *service, port_t port, int type, int protocol, long timeout, int rcvbufsz, int sndbufsz, sockaddr_t *addr, size_t *addrsize)>

Creates a client socket of the specified C<type> (e.g. C<SOCK_STREAM> or
C<SOCK_DGRAM>) and C<protocol> (usually zero) and connects to the server
listening on C<host> (as determined by I<gethostbyname(3)>) on the port
number specified by C<service> (as determined by C<getservbyname(3)>) or by
C<port> (which must be in host byte order) otherwise.
...
If C<addr> is not C<NULL>, the address of the server is stored at that
address. If C<addrsize> is not C<NULL>, the length of C<addr> is stored at
that address.
...
int net_create_client(const char *host, const char *service, port_t port, int type, int protocol, long timeout, int rcvbufsz, int sndbufsz, sockaddr_t *addr, size_t *addrsize)
{
	...
	/* Create the socket. If connectionless, bind */
	...
	if (type == SOCK_DGRAM) /* This isn't really needed for UDP */
	{
		...
		if (bind(sockfd, (sockaddr_t *)&localip, sizeof localip) == -1)
			return close(sockfd), -1;
	}

	/* Connect to ipaddr (possibly with a timeout) */

	if (timeout && nonblock_on(sockfd) == -1)
		return close(sockfd), -1;

	if ((rc = connect(sockfd, ipaddr, ipsize)) == -1 && errno != EINPROGRESS)
		return close(sockfd), -1;
...
and sends it on the connected socket, C<sockfd>, with I<send(2)>. C<flags>
is passed to I<send(2)>. This is intended for use with UDP. It can work
reliably with TCP but only when the application protocol involves each peer
packing and unpacking alternatively, each waiting for the other's response
before making their next response. On success, returns the number of bytes
packed and sent. On error, returns -1 with C<errno> set appropriately.
...
it. It is much faster to not copy the data at all. When possible, use
I<writev(2)> instead to write multiple non-contiguous buffers in a single
system call.
...
=item C<ssize_t net_packto(int sockfd, int flags, const sockaddr_t *to, size_t tolen, const char *fmt, ...)>
...
=item C<ssize_t net_vpackto(int sockfd, int flags, const sockaddr_t *to, size_t tolen, const char *fmt, va_list args)>
...
When possible, use I<readv(2)> instead to read into multiple non-contiguous
buffers in a single system call.
...
=item C<ssize_t net_unpackfrom(int sockfd, int flags, sockaddr_t *from, size_t *fromlen, const char *fmt, ...)>
...
=item C<ssize_t net_vunpackfrom(int sockfd, int flags, sockaddr_t *from, size_t *fromlen, const char *fmt, va_list args)>
...
no non nul-terminated strings or machine dependant formats or uuencoding or
BER integer compression, everything is in network byte order, and floats are
represented as strings so I<pack()> is suitable for serialising data to be
written to disk or sent across a network to other hosts. OK, C<v> and C<w>
...
from the C<size_t> object pointed to by the next argument and the size of
...
=item C<ssize_t net_read(int sockfd, long timeout, const char *buf, size_t count)>
...
number of bytes read. On error, returns -1 with C<errno> set appropriately.
...
closes, returns 0. On error, returns -1 with C<errno> set appropriately.
...
C<timeout> is the number of seconds to wait before timing out. On success,
returns the number of bytes written. On error, returns -1 with C<errno> set
appropriately.
...
int mail(const char *server, const char *sender, const char *recipients, const char *subject, const char *message)
{
	...
	smtp = net_client(server ? server : "localhost", "smtp", 25, 30, 0, 0, NULL, NULL);
	if (smtp == -1)
		return -1;
...
MT-Safe except for the net server/client functions which use
I<getprotobynumber(3)>, I<getservbyname(3)> and I<gethostbyname(3)>
which are all non-reentrant. This will be fixed.
...
A TCP client:
    ...
    int main()
    {
        int servfd = net_client("localhost", NULL, 30000, 5, 0, 0, NULL, NULL);
        if (servfd == -1)
            return 1;
...
A UDP server:
    ...
    int main()
    {
        ...
        for (;;)
        {
            if (recvfrom(servfd, pkt, 8, 0, (sockaddr_t *)&ipaddr, &ipsize) == -1)
                return 1;

            provide_service(pkt);

            if (sendto(servfd, pkt, 8, 0, (sockaddr_t *)&ipaddr, ipsize) == -1)
                return 1;
        }
    }
...
A UDP client:
    ...
    {
        char pkt[8];
        int servfd = net_udp_client("localhost", NULL, 30000, 0, 0, NULL, NULL);
        if (servfd == -1)
            return 1;
...
Pack and unpack a packet with a length field:
    ...
    unpacked = unpack(pkt, packed, "ia?", &len, &len, sizeof(data), data);
...
addresses on systems (e.g. Linux) that support IPv6). They probably should
support X.25, SNA and UNIX sockets. When creating a server that binds to all
local network interfaces, only IPv4 addresses are supported.
...
int main(int ac, char **av)
{
	...
	server = net_server("localhost", NULL, 30000, 0, 0, NULL, NULL);
	if (server == -1)
	...
	else
	{
		switch (pid = fork())
		{
			...
			default:
			{
				...
				if (read_timeout(server, 5, 0) == -1 || (s = accept(server, (sockaddr_t *)addr, &addrlen)) == -1)
			...
			case 0:
			{
				...
				for (i = 0; i < 5; ++i)
				{
					c = net_client("localhost", NULL, 30000, 5, 0, 0, NULL, NULL);
					if (c != -1)
						break;
					sleep(1);
				}
				...
				else
				{
					...
					if (write_timeout(c, 5, 0) == -1 || write(c, "HELO", 4) == -1)
						++errors, printf("Test8: write(c, HELO) failed (%s)\n", strerror(errno));
					else if (read_timeout(c, 5, 0) == -1 || read(c, test, 4) == -1)
	...
	server = net_udp_server("localhost", NULL, 30000, 0, 0, NULL, NULL);
	if (server == -1)
	...
	else
	{
		switch (pid = fork())
		{
			...
			default:
			{
				...
				if (read_timeout(server, 5, 0) == -1 || recvfrom(server, test, 4, 0, (sockaddr_t *)&ipaddr, &ipsize) == -1)
					++errors, printf("Test14: recvfrom(server, HELO) failed (%s)\n", strerror(errno));
				else if (memcmp(test, "HELO", 4))
					++errors, printf("Test15: recvfrom(server, HELO) failed (recv \"%4.4s\", not \"%4.4s\")\n", test, "HELO");
				else if (write_timeout(server, 5, 0) == -1 || sendto(server, "OLEH", 4, 0, (sockaddr_t *)&ipaddr, ipsize) == -1)
			...
			case 0:
			{
				...
				for (i = 0; i < 5; ++i)
				{
					c = net_udp_client("localhost", NULL, 30000, 0, 0, NULL, NULL);
					if (c != -1)
						break;
					sleep(1);
				}
				...
				if (c == -1)
					++errors, printf("Test17: net_udp_client(\"localhost\", 30000) failed (%s)\n", strerror(errno));
				else
				{
					char test[4];

					if (write_timeout(c, 5, 0) == -1 || send(c, "HELO", 4, 0) == -1)
						++errors, printf("Test18: send(c, HELO) failed (%s)\n", strerror(errno));
					else if (read_timeout(c, 5, 0) == -1 || recv(c, test, 4, 0) == -1)
...
#define TEST_VARSIZE(i, pfmt, ufmt, size, len1, data1, len2ref, data2limit, data2) \
	...
	b2 = malloc(strlen(b) + 1);
	h2 = malloc(strlen(h) + 1);
	length = 10; TEST_VARSIZE(101, "ia*", "ia?", 14, 10, a, &length, 10, a2)
	length = 11; TEST_VARSIZE(102, "iz*", "iz?", 15, 11, z, &length, 11, z2)
	length = 18; TEST_VARSIZE(103, "ib*", "ib?", 7, 18, b, &length, 18, b2)
	length = 16; TEST_VARSIZE(104, "ih*", "ih?", 12, 16, h, &length, 16, h2)
	free(a2);
	free(z2);
	free(b2);
	free(h2);
...
#define TEST_FAILURE(i, test, error) \
	...
	TEST_FAILURE(105, pack(NULL, 1, "a", a), EINVAL)
	TEST_FAILURE(106, pack(pkt, 1, NULL, a), EINVAL)

	TEST_FAILURE(107, pack(pkt, 1, "a0", a), EINVAL)
	TEST_FAILURE(108, pack(pkt, 1, "a*", 0, a), EINVAL)
	TEST_FAILURE(109, pack(pkt, 1, "a*", -1, a), EINVAL)
	TEST_FAILURE(110, pack(pkt, 0, "a", a), ENOSPC)
	TEST_FAILURE(111, pack(pkt, 1, "a2", a), ENOSPC)
	TEST_FAILURE(112, pack(pkt, 1, "a*", 2, a), ENOSPC)
	TEST_FAILURE(113, pack(pkt, 1, "a", NULL), EINVAL)

	TEST_FAILURE(114, pack(pkt, 1, "z0", z), EINVAL)
	TEST_FAILURE(115, pack(pkt, 1, "z*", 0, z), EINVAL)
	TEST_FAILURE(116, pack(pkt, 1, "z*", -1, z), EINVAL)
	TEST_FAILURE(117, pack(pkt, 0, "z", z), ENOSPC)
	TEST_FAILURE(118, pack(pkt, 1, "z2", z), ENOSPC)
	TEST_FAILURE(119, pack(pkt, 1, "z*", 2, z), ENOSPC)
	TEST_FAILURE(120, pack(pkt, 1, "z", NULL), EINVAL)

	TEST_FAILURE(121, pack(pkt, 1, "b0", b), EINVAL)
	TEST_FAILURE(122, pack(pkt, 1, "b*", 0, b), EINVAL)
	TEST_FAILURE(123, pack(pkt, 1, "b*", -1, b), EINVAL)
	TEST_FAILURE(124, pack(pkt, 0, "b", b), ENOSPC)
	TEST_FAILURE(125, pack(pkt, 1, "b9", b), ENOSPC)
	TEST_FAILURE(126, pack(pkt, 1, "b*", 9, b), ENOSPC)
	TEST_FAILURE(127, pack(pkt, 1, "b", NULL), EINVAL)
	TEST_FAILURE(128, pack(pkt, 1, "b8", a), EINVAL)
	TEST_FAILURE(129, pack(pkt, 1, "b8", h), EINVAL)

	TEST_FAILURE(130, pack(pkt, 1, "h0", h), EINVAL)
	TEST_FAILURE(131, pack(pkt, 1, "h*", 0, h), EINVAL)
	TEST_FAILURE(132, pack(pkt, 1, "h*", -1, h), EINVAL)
	TEST_FAILURE(133, pack(pkt, 0, "h", h), ENOSPC)
	TEST_FAILURE(134, pack(pkt, 1, "h3", h), ENOSPC)
	TEST_FAILURE(135, pack(pkt, 1, "h*", 3, h), ENOSPC)
	TEST_FAILURE(136, pack(pkt, 1, "h", NULL), EINVAL)
	TEST_FAILURE(137, pack(pkt, 1, "h", a), EINVAL)

	TEST_FAILURE(138, pack(pkt, 1, "c0", sc), EINVAL)
	TEST_FAILURE(139, pack(pkt, 1, "c*", 0, sc), EINVAL)
	TEST_FAILURE(140, pack(pkt, 1, "c*", -1, sc), EINVAL)
	TEST_FAILURE(141, pack(pkt, 0, "c", sc), ENOSPC)
	TEST_FAILURE(142, pack(pkt, 1, "c2", sc, sc), ENOSPC)
	TEST_FAILURE(143, pack(pkt, 1, "c*", 2, sc, sc), ENOSPC)

	TEST_FAILURE(144, pack(pkt, 1, "c0", uc), EINVAL)
	TEST_FAILURE(145, pack(pkt, 1, "c*", 0, uc), EINVAL)
	TEST_FAILURE(146, pack(pkt, 1, "c*", -1, uc), EINVAL)
	TEST_FAILURE(147, pack(pkt, 0, "c", uc), ENOSPC)
	TEST_FAILURE(148, pack(pkt, 1, "c2", uc, uc), ENOSPC)
	TEST_FAILURE(149, pack(pkt, 1, "c*", 2, uc, uc), ENOSPC)

	TEST_FAILURE(150, pack(pkt, 1, "s0", ss), EINVAL)
	TEST_FAILURE(151, pack(pkt, 1, "s*", 0, ss), EINVAL)
	TEST_FAILURE(152, pack(pkt, 1, "s*", -1, ss), EINVAL)
	TEST_FAILURE(153, pack(pkt, 0, "s", ss), ENOSPC)
	TEST_FAILURE(154, pack(pkt, 1, "s", ss), ENOSPC)
	TEST_FAILURE(155, pack(pkt, 2, "s2", ss, ss), ENOSPC)
	TEST_FAILURE(156, pack(pkt, 3, "s*", 2, ss, ss), ENOSPC)

	TEST_FAILURE(157, pack(pkt, 1, "s0", us), EINVAL)
	TEST_FAILURE(158, pack(pkt, 1, "s*", 0, us), EINVAL)
	TEST_FAILURE(159, pack(pkt, 1, "s*", -1, us), EINVAL)
	TEST_FAILURE(160, pack(pkt, 0, "s", us), ENOSPC)
	TEST_FAILURE(161, pack(pkt, 1, "s", us), ENOSPC)
	TEST_FAILURE(162, pack(pkt, 2, "s2", us, us), ENOSPC)
	TEST_FAILURE(163, pack(pkt, 3, "s*", 2, us, us), ENOSPC)

	TEST_FAILURE(164, pack(pkt, 1, "i0", si), EINVAL)
	TEST_FAILURE(165, pack(pkt, 1, "i*", 0, si), EINVAL)
	TEST_FAILURE(166, pack(pkt, 1, "i*", -1, si), EINVAL)
	TEST_FAILURE(167, pack(pkt, 0, "i", si), ENOSPC)
	TEST_FAILURE(168, pack(pkt, 1, "i", si), ENOSPC)
	TEST_FAILURE(169, pack(pkt, 2, "i", si), ENOSPC)
	TEST_FAILURE(170, pack(pkt, 3, "i", si), ENOSPC)
	TEST_FAILURE(171, pack(pkt, 4, "i2", si, si), ENOSPC)
	TEST_FAILURE(172, pack(pkt, 5, "i*", 2, si, si), ENOSPC)

	TEST_FAILURE(173, pack(pkt, 1, "i0", ui), EINVAL)
	TEST_FAILURE(174, pack(pkt, 1, "i*", 0, ui), EINVAL)
	TEST_FAILURE(175, pack(pkt, 1, "i*", -1, ui), EINVAL)
	TEST_FAILURE(176, pack(pkt, 0, "i", ui), ENOSPC)
	TEST_FAILURE(177, pack(pkt, 1, "i", ui), ENOSPC)
	TEST_FAILURE(178, pack(pkt, 2, "i", ui), ENOSPC)
	TEST_FAILURE(179, pack(pkt, 3, "i", ui), ENOSPC)
	TEST_FAILURE(180, pack(pkt, 4, "i2", ui, ui), ENOSPC)
	TEST_FAILURE(181, pack(pkt, 5, "i*", 2, ui, ui), ENOSPC)

	TEST_FAILURE(182, pack(pkt, 1, "v0", sv1), EINVAL)
	TEST_FAILURE(183, pack(pkt, 1, "v*", 0, sv1), EINVAL)
	TEST_FAILURE(184, pack(pkt, 1, "v*", -1, sv1), EINVAL)
	TEST_FAILURE(185, pack(pkt, 0, "v", sv1), ENOSPC)
	TEST_FAILURE(186, pack(pkt, 1, "v", sv1), ENOSPC)
	TEST_FAILURE(187, pack(pkt, 2, "v2", sv1, sv2), ENOSPC)
	TEST_FAILURE(188, pack(pkt, 3, "v*", 2, sv1, sv2), ENOSPC)

	TEST_FAILURE(189, pack(pkt, 1, "v0", uv1), EINVAL)
	TEST_FAILURE(190, pack(pkt, 1, "v*", 0, uv1), EINVAL)
	TEST_FAILURE(191, pack(pkt, 1, "v*", -1, uv1), EINVAL)
	TEST_FAILURE(192, pack(pkt, 0, "v", uv1), ENOSPC)
	TEST_FAILURE(193, pack(pkt, 1, "v", uv1), ENOSPC)
	TEST_FAILURE(194, pack(pkt, 2, "v2", uv1, uv2), ENOSPC)
	TEST_FAILURE(195, pack(pkt, 3, "v*", 2, uv1, uv2), ENOSPC)

	TEST_FAILURE(196, pack(pkt, 1, "w0", sw1), EINVAL)
	TEST_FAILURE(197, pack(pkt, 1, "w*", 0, sw1), EINVAL)
	TEST_FAILURE(198, pack(pkt, 1, "w*", -1, sw1), EINVAL)
	TEST_FAILURE(199, pack(pkt, 0, "w", sw1), ENOSPC)
	TEST_FAILURE(200, pack(pkt, 1, "w", sw1), ENOSPC)
	TEST_FAILURE(201, pack(pkt, 2, "w", uv1), ENOSPC)
	TEST_FAILURE(202, pack(pkt, 3, "w", uv1), ENOSPC)
	TEST_FAILURE(203, pack(pkt, 4, "w2", sw1, sw2), ENOSPC)
	TEST_FAILURE(204, pack(pkt, 5, "w*", 2, sw1, sw2), ENOSPC)

	TEST_FAILURE(205, pack(pkt, 1, "w0", uw1), EINVAL)
	TEST_FAILURE(206, pack(pkt, 1, "w*", 0, uw1), EINVAL)
	TEST_FAILURE(207, pack(pkt, 1, "w*", -1, uw1), EINVAL)
	TEST_FAILURE(208, pack(pkt, 0, "w", uw1), ENOSPC)
	TEST_FAILURE(209, pack(pkt, 1, "w", uw1), ENOSPC)
	TEST_FAILURE(210, pack(pkt, 2, "w", uw1), ENOSPC)
	TEST_FAILURE(211, pack(pkt, 3, "w", uw1), ENOSPC)
	TEST_FAILURE(212, pack(pkt, 4, "w2", uw1, uw2), ENOSPC)
	TEST_FAILURE(213, pack(pkt, 5, "w*", 2, uw1, uw2), ENOSPC)

	TEST_FAILURE(214, pack(pkt, 1, "p0", p), EINVAL)
	TEST_FAILURE(215, pack(pkt, 1, "p*", 0, p), EINVAL)
	TEST_FAILURE(216, pack(pkt, 1, "p*", -1, p), EINVAL)
	TEST_FAILURE(217, pack(pkt, 0, "p", p), ENOSPC)
	TEST_FAILURE(218, pack(pkt, 1, "p", p), ENOSPC)
	TEST_FAILURE(219, pack(pkt, 2, "p", p), ENOSPC)
	TEST_FAILURE(220, pack(pkt, 3, "p", p), ENOSPC)
	TEST_FAILURE(221, pack(pkt, 4, "p2", p, p), ENOSPC)
	TEST_FAILURE(222, pack(pkt, 5, "p*", 2, p, p), ENOSPC)

#ifdef WANT_LONG_LONG
	TEST_FAILURE(223, pack(pkt, 1, "l0", sl), EINVAL)
	TEST_FAILURE(224, pack(pkt, 1, "l*", 0, sl), EINVAL)
	TEST_FAILURE(225, pack(pkt, 1, "l*", -1, sl), EINVAL)
	TEST_FAILURE(226, pack(pkt, 0, "l", sl), ENOSPC)
	TEST_FAILURE(227, pack(pkt, 1, "l", sl), ENOSPC)
	TEST_FAILURE(228, pack(pkt, 2, "l", sl), ENOSPC)
	TEST_FAILURE(229, pack(pkt, 3, "l", sl), ENOSPC)
	TEST_FAILURE(230, pack(pkt, 4, "l", sl), ENOSPC)
	TEST_FAILURE(231, pack(pkt, 5, "l", sl), ENOSPC)
	TEST_FAILURE(232, pack(pkt, 6, "l", sl), ENOSPC)
	TEST_FAILURE(233, pack(pkt, 7, "l", sl), ENOSPC)
	TEST_FAILURE(234, pack(pkt, 8, "l2", sl, sl), ENOSPC)
	TEST_FAILURE(235, pack(pkt, 9, "l*", 2, sl, sl), ENOSPC)

	TEST_FAILURE(236, pack(pkt, 1, "l0", ul), EINVAL)
	TEST_FAILURE(237, pack(pkt, 1, "l*", 0, ul), EINVAL)
	TEST_FAILURE(238, pack(pkt, 1, "l*", -1, ul), EINVAL)
	TEST_FAILURE(239, pack(pkt, 0, "l", ul), ENOSPC)
	TEST_FAILURE(240, pack(pkt, 1, "l", ul), ENOSPC)
	TEST_FAILURE(241, pack(pkt, 2, "l", ul), ENOSPC)
	TEST_FAILURE(242, pack(pkt, 3, "l", ul), ENOSPC)
	TEST_FAILURE(243, pack(pkt, 4, "l", ul), ENOSPC)
	TEST_FAILURE(244, pack(pkt, 5, "l", ul), ENOSPC)
	TEST_FAILURE(245, pack(pkt, 6, "l", ul), ENOSPC)
	TEST_FAILURE(246, pack(pkt, 7, "l", ul), ENOSPC)
	TEST_FAILURE(247, pack(pkt, 8, "l2", ul, ul), ENOSPC)
	TEST_FAILURE(248, pack(pkt, 9, "l*", 2, ul, ul), ENOSPC)
#else
	TEST_FAILURE(249, pack(pkt, 8, "l", 0), ENOSYS)
	TEST_FAILURE(250, pack(pkt, 8, "l", 0), ENOSYS)
#endif

	f = -1.5; /* packed length = 6 */
	TEST_FAILURE(251, pack(pkt, 1, "f0", f), EINVAL)
	TEST_FAILURE(252, pack(pkt, 1, "f*", 0, f), EINVAL)
	TEST_FAILURE(253, pack(pkt, 1, "f*", -1, f), EINVAL)
	TEST_FAILURE(254, pack(pkt, 0, "f", f), ENOSPC)
	TEST_FAILURE(255, pack(pkt, 1, "f", f), ENOSPC)
	TEST_FAILURE(256, pack(pkt, 2, "f", f), ENOSPC)
	TEST_FAILURE(257, pack(pkt, 3, "f", f), ENOSPC)
	TEST_FAILURE(258, pack(pkt, 4, "f", f), ENOSPC)
	TEST_FAILURE(259, pack(pkt, 5, "f", f), ENOSPC)
	TEST_FAILURE(260, pack(pkt, 6, "f2", f, f), ENOSPC)
	TEST_FAILURE(261, pack(pkt, 7, "f*", 2, f, f), ENOSPC)

	da = db = 1.5; /* packed length = 5 */
	TEST_FAILURE(262, pack(pkt, 1, "d0", da), EINVAL)
	TEST_FAILURE(263, pack(pkt, 1, "d*", 0, da), EINVAL)
	TEST_FAILURE(264, pack(pkt, 1, "d*", -1, da), EINVAL)
	TEST_FAILURE(265, pack(pkt, 0, "d", da), ENOSPC)
	TEST_FAILURE(266, pack(pkt, 1, "d", da), ENOSPC)
	TEST_FAILURE(267, pack(pkt, 2, "d", da), ENOSPC)
	TEST_FAILURE(268, pack(pkt, 3, "d", da), ENOSPC)
	TEST_FAILURE(269, pack(pkt, 4, "d", da), ENOSPC)
	TEST_FAILURE(270, pack(pkt, 5, "d2", da, db), ENOSPC)
	TEST_FAILURE(271, pack(pkt, 6, "d*", 2, da, db), ENOSPC)

	TEST_FAILURE(272, pack(pkt, 1, "x0"), EINVAL)
	TEST_FAILURE(273, pack(pkt, 1, "x*", 0), EINVAL)
	TEST_FAILURE(274, pack(pkt, 1, "x*", -1), EINVAL)
	TEST_FAILURE(275, pack(pkt, 0, "x"), ENOSPC)
	TEST_FAILURE(276, pack(pkt, 1, "x2"), ENOSPC)
	TEST_FAILURE(277, pack(pkt, 1, "x*", 2), ENOSPC)

	TEST_FAILURE(278, pack(pkt, 1, "X0"), EINVAL)
	TEST_FAILURE(279, pack(pkt, 1, "X*", 0), EINVAL)
	TEST_FAILURE(280, pack(pkt, 1, "X*", -1), EINVAL)
	TEST_FAILURE(281, pack(pkt, 0, "X"), EINVAL)
	TEST_FAILURE(282, pack(pkt, 1, "X"), EINVAL)
	TEST_FAILURE(283, pack(pkt, 1, "X2"), EINVAL)
	TEST_FAILURE(284, pack(pkt, 1024, "X*", 2), EINVAL)

	TEST_FAILURE(285, pack(pkt, 1, "@0"), EINVAL)
	TEST_FAILURE(286, pack(pkt, 1, "@*", 0), EINVAL)
	TEST_FAILURE(287, pack(pkt, 1, "@*", -1), EINVAL)
	TEST_FAILURE(288, pack(pkt, 0, "@"), ENOSPC)
	TEST_FAILURE(289, pack(pkt, 0, "@1"), ENOSPC)
	TEST_FAILURE(290, pack(pkt, 1, "@2"), ENOSPC)
	TEST_FAILURE(291, pack(pkt, 1, "@*", 2), ENOSPC)

	TEST_FAILURE(292, pack(pkt, 1, "?", 0), EINVAL)

	length = 11;
	TEST_FAILURE(293, unpack(NULL, 1, "a", a), EINVAL)
	TEST_FAILURE(294, unpack(pkt, 1, NULL, a), EINVAL)

	TEST_FAILURE(295, unpack(pkt, 1, "a0", a), EINVAL)
	TEST_FAILURE(296, unpack(pkt, 1, "a*", 0, a), EINVAL)
	TEST_FAILURE(297, unpack(pkt, 1, "a*", -1, a), EINVAL)
	TEST_FAILURE(298, unpack(pkt, 0, "a", a), ENOSPC)
	TEST_FAILURE(299, unpack(pkt, 1, "a2", a), ENOSPC)
	TEST_FAILURE(300, unpack(pkt, 1, "a*", 2, a), ENOSPC)
	TEST_FAILURE(301, unpack(pkt, 0, "a", NULL), ENOSPC)
	TEST_FAILURE(302, unpack(pkt, 10, "a?", NULL, 10, a), EINVAL)
	TEST_FAILURE(303, unpack(pkt, 10, "a?", &length, 10, a), ENOSPC)

	TEST_FAILURE(304, unpack(pkt, 1, "z0", z), EINVAL)
	TEST_FAILURE(305, unpack(pkt, 1, "z*", 0, z), EINVAL)
	TEST_FAILURE(306, unpack(pkt, 1, "z*", -1, z), EINVAL)
	TEST_FAILURE(307, unpack(pkt, 0, "z", z), ENOSPC)
	TEST_FAILURE(308, unpack(pkt, 1, "z2", z), ENOSPC)
	TEST_FAILURE(309, unpack(pkt, 1, "z*", 2, z), ENOSPC)
	TEST_FAILURE(310, unpack(pkt, 0, "z", NULL), ENOSPC)
	TEST_FAILURE(311, unpack(pkt, 10, "z?", NULL, 10, z), EINVAL)
	TEST_FAILURE(312, unpack(pkt, 10, "z?", &length, 10, z), ENOSPC)

	TEST_FAILURE(313, unpack(pkt, 1, "b0", b), EINVAL)
	TEST_FAILURE(314, unpack(pkt, 1, "b*", 0, b), EINVAL)
	TEST_FAILURE(315, unpack(pkt, 1, "b*", -1, b), EINVAL)
	TEST_FAILURE(316, unpack(pkt, 0, "b", b), ENOSPC)
	TEST_FAILURE(317, unpack(pkt, 1, "b9", b), ENOSPC)
	TEST_FAILURE(318, unpack(pkt, 1, "b*", 9, b), ENOSPC)
	TEST_FAILURE(319, unpack(pkt, 0, "b", NULL), ENOSPC)
	TEST_FAILURE(320, unpack(pkt, 10, "b?", NULL, 10, b), EINVAL)
	TEST_FAILURE(321, unpack(pkt, 10, "b?", &length, 10, b), ENOSPC)

	TEST_FAILURE(322, unpack(pkt, 1, "h0", h), EINVAL)
	TEST_FAILURE(323, unpack(pkt, 1, "h*", 0, h), EINVAL)
	TEST_FAILURE(324, unpack(pkt, 1, "h*", -1, h), EINVAL)
	TEST_FAILURE(325, unpack(pkt, 0, "h", h), ENOSPC)
	TEST_FAILURE(326, unpack(pkt, 1, "h3", h), ENOSPC)
	TEST_FAILURE(327, unpack(pkt, 1, "h*", 3, h), ENOSPC)
	TEST_FAILURE(328, unpack(pkt, 0, "h", NULL), ENOSPC)
	TEST_FAILURE(329, unpack(pkt, 10, "h?", NULL, 10, h), EINVAL)
	TEST_FAILURE(330, unpack(pkt, 10, "h?", &length, 10, h), ENOSPC)

	TEST_FAILURE(331, unpack(pkt, 1, "c0", &sc), EINVAL)
	TEST_FAILURE(332, unpack(pkt, 1, "c*", 0, &sc), EINVAL)
	TEST_FAILURE(333, unpack(pkt, 1, "c*", -1, &sc), EINVAL)
	TEST_FAILURE(334, unpack(pkt, 0, "c", &sc), ENOSPC)
	TEST_FAILURE(335, unpack(pkt, 1, "c2", &sc, &sc), ENOSPC)
	TEST_FAILURE(336, unpack(pkt, 1, "c*", 2, &sc, &sc), ENOSPC)
	TEST_FAILURE(337, unpack(pkt, 1, "c?", &length, 1, &sc), EINVAL)

	TEST_FAILURE(338, unpack(pkt, 1, "c0", &uc), EINVAL)
	TEST_FAILURE(339, unpack(pkt, 1, "c*", 0, &uc), EINVAL)
	TEST_FAILURE(340, unpack(pkt, 1, "c*", -1, &uc), EINVAL)
	TEST_FAILURE(341, unpack(pkt, 0, "c", &uc), ENOSPC)
	TEST_FAILURE(342, unpack(pkt, 1, "c2", &uc, &uc), ENOSPC)
	TEST_FAILURE(343, unpack(pkt, 1, "c*", 2, &uc, &uc), ENOSPC)

	TEST_FAILURE(344, unpack(pkt, 1, "s0", &ss), EINVAL)
	TEST_FAILURE(345, unpack(pkt, 1, "s*", 0, &ss), EINVAL)
	TEST_FAILURE(346, unpack(pkt, 1, "s*", -1, &ss), EINVAL)
	TEST_FAILURE(347, unpack(pkt, 0, "s", &ss), ENOSPC)
	TEST_FAILURE(348, unpack(pkt, 1, "s", &ss), ENOSPC)
	TEST_FAILURE(349, unpack(pkt, 2, "s2", &ss, &ss), ENOSPC)
	TEST_FAILURE(350, unpack(pkt, 3, "s*", 2, &ss, &ss), ENOSPC)
	TEST_FAILURE(351, unpack(pkt, 2, "s?", &length, 1, &ss), EINVAL)

	TEST_FAILURE(352, unpack(pkt, 1, "s0", &us), EINVAL)
	TEST_FAILURE(353, unpack(pkt, 1, "s*", 0, &us), EINVAL)
	TEST_FAILURE(354, unpack(pkt, 1, "s*", -1, &us), EINVAL)
	TEST_FAILURE(355, unpack(pkt, 0, "s", &us), ENOSPC)
	TEST_FAILURE(356, unpack(pkt, 1, "s", &us), ENOSPC)
	TEST_FAILURE(357, unpack(pkt, 2, "s2", &us, &us), ENOSPC)
	TEST_FAILURE(358, unpack(pkt, 3, "s*", 2, &us, &us), ENOSPC)

	TEST_FAILURE(359, unpack(pkt, 1, "i0", &si), EINVAL)
	TEST_FAILURE(360, unpack(pkt, 1, "i*", 0, &si), EINVAL)
	TEST_FAILURE(361, unpack(pkt, 1, "i*", -1, &si), EINVAL)
	TEST_FAILURE(362, unpack(pkt, 0, "i", &si), ENOSPC)
	TEST_FAILURE(363, unpack(pkt, 1, "i", &si), ENOSPC)
	TEST_FAILURE(364, unpack(pkt, 2, "i", &si), ENOSPC)
	TEST_FAILURE(365, unpack(pkt, 3, "i", &si), ENOSPC)
	TEST_FAILURE(366, unpack(pkt, 4, "i2", &si, &si), ENOSPC)
	TEST_FAILURE(367, unpack(pkt, 5, "i*", 2, &si, &si), ENOSPC)
	TEST_FAILURE(368, unpack(pkt, 4, "i?", &length, 1, &si), EINVAL)

	TEST_FAILURE(369, unpack(pkt, 1, "i0", &ui), EINVAL)
	TEST_FAILURE(370, unpack(pkt, 1, "i*", 0, &ui), EINVAL)
	TEST_FAILURE(371, unpack(pkt, 1, "i*", -1, &ui), EINVAL)
	TEST_FAILURE(372, unpack(pkt, 0, "i", &ui), ENOSPC)
	TEST_FAILURE(373, unpack(pkt, 1, "i", &ui), ENOSPC)
	TEST_FAILURE(374, unpack(pkt, 2, "i", &ui), ENOSPC)
	TEST_FAILURE(375, unpack(pkt, 3, "i", &ui), ENOSPC)
	TEST_FAILURE(376, unpack(pkt, 4, "i2", &ui, &ui), ENOSPC)
	TEST_FAILURE(377, unpack(pkt, 5, "i*", 2, &ui, &ui), ENOSPC)

	TEST_FAILURE(378, unpack(pkt, 1, "p0", &p), EINVAL)
	TEST_FAILURE(379, unpack(pkt, 1, "p*", 0, &p), EINVAL)
	TEST_FAILURE(380, unpack(pkt, 1, "p*", -1, &p), EINVAL)
	TEST_FAILURE(381, unpack(pkt, 0, "p", &p), ENOSPC)
	TEST_FAILURE(382, unpack(pkt, 1, "p", &p), ENOSPC)
	TEST_FAILURE(383, unpack(pkt, 2, "p", &p), ENOSPC)
	TEST_FAILURE(384, unpack(pkt, 3, "p", &p), ENOSPC)
	TEST_FAILURE(385, unpack(pkt, 4, "p2", &p, &p2), ENOSPC)
	TEST_FAILURE(386, unpack(pkt, 5, "p*", 2, &p, &p2), ENOSPC)
	TEST_FAILURE(387, unpack(pkt, 4, "p?", &length, 1, &p), EINVAL)

#ifdef WANT_LONG_LONG
	TEST_FAILURE(388, unpack(pkt, 1, "l0", &sl), EINVAL)
	TEST_FAILURE(389, unpack(pkt, 1, "l*", 0, &sl), EINVAL)
	TEST_FAILURE(390, unpack(pkt, 1, "l*", -1, &sl), EINVAL)
	TEST_FAILURE(391, unpack(pkt, 0, "l", &sl), ENOSPC)
	TEST_FAILURE(392, unpack(pkt, 1, "l", &sl), ENOSPC)
	TEST_FAILURE(393, unpack(pkt, 2, "l", &sl), ENOSPC)
	TEST_FAILURE(394, unpack(pkt, 3, "l", &sl), ENOSPC)
	TEST_FAILURE(395, unpack(pkt, 4, "l", &sl), ENOSPC)
	TEST_FAILURE(396, unpack(pkt, 5, "l", &sl), ENOSPC)
	TEST_FAILURE(397, unpack(pkt, 6, "l", &sl), ENOSPC)
	TEST_FAILURE(398, unpack(pkt, 7, "l", &sl), ENOSPC)
	TEST_FAILURE(399, unpack(pkt, 8, "l2", &sl, &sl), ENOSPC)
	TEST_FAILURE(400, unpack(pkt, 9, "l*", 2, &sl, &sl), ENOSPC)
	TEST_FAILURE(401, unpack(pkt, 8, "l?", &length, 1, &sl), EINVAL)

	TEST_FAILURE(402, unpack(pkt, 1, "l0", &ul), EINVAL)
	TEST_FAILURE(403, unpack(pkt, 1, "l*", 0, &ul), EINVAL)
	TEST_FAILURE(404, unpack(pkt, 1, "l*", -1, &ul), EINVAL)
	TEST_FAILURE(405, unpack(pkt, 0, "l", &ul), ENOSPC)
	TEST_FAILURE(406, unpack(pkt, 1, "l", &ul), ENOSPC)
	TEST_FAILURE(407, unpack(pkt, 2, "l", &ul), ENOSPC)
	TEST_FAILURE(408, unpack(pkt, 3, "l", &ul), ENOSPC)
	TEST_FAILURE(409, unpack(pkt, 4, "l", &ul), ENOSPC)
	TEST_FAILURE(410, unpack(pkt, 5, "l", &ul), ENOSPC)
	TEST_FAILURE(411, unpack(pkt, 6, "l", &ul), ENOSPC)
	TEST_FAILURE(412, unpack(pkt, 7, "l", &ul), ENOSPC)
	TEST_FAILURE(413, unpack(pkt, 8, "l2", &ul, &ul), ENOSPC)
	TEST_FAILURE(414, unpack(pkt, 9, "l*", 2, &ul, &ul), ENOSPC)
#else
	TEST_FAILURE(415, unpack(pkt, 8, "l", NULL), ENOSYS)
	TEST_FAILURE(416, unpack(pkt, 8, "l", NULL), ENOSYS)
#endif

	f = -1.5; /* packed length = 6 */
	pack(pkt, 1024, "ff", f, f); /* needs this to find length */
	TEST_FAILURE(417, unpack(pkt, 1, "f0", &f), EINVAL)
	TEST_FAILURE(418, unpack(pkt, 1, "f*", 0, &f), EINVAL)
	TEST_FAILURE(419, unpack(pkt, 1, "f*", -1, &f), EINVAL)
	TEST_FAILURE(420, unpack(pkt, 0, "f", &f), ENOSPC)
	TEST_FAILURE(421, unpack(pkt, 1, "f", &f), ENOSPC)
	TEST_FAILURE(422, unpack(pkt, 2, "f", &f), ENOSPC)
	TEST_FAILURE(423, unpack(pkt, 3, "f", &f), ENOSPC)
	TEST_FAILURE(424, unpack(pkt, 4, "f", &f), ENOSPC)
	TEST_FAILURE(425, unpack(pkt, 5, "f", &f), ENOSPC)
	TEST_FAILURE(426, unpack(pkt, 6, "f2", &f, &f), ENOSPC)
	TEST_FAILURE(427, unpack(pkt, 7, "f*", 2, &f, &f), ENOSPC)
	TEST_FAILURE(428, unpack(pkt, 10, "f?", &length, 1, &f), EINVAL)

	da = db = 1.5; /* packed length = 5 */
	pack(pkt, 1024, "dd", da, db); /* needs this to find length */
	TEST_FAILURE(429, unpack(pkt, 1, "d0", &da), EINVAL)
	TEST_FAILURE(430, unpack(pkt, 1, "d*", 0, &da), EINVAL)
	TEST_FAILURE(431, unpack(pkt, 1, "d*", -1, &da), EINVAL)
	TEST_FAILURE(432, unpack(pkt, 0, "d", &da), ENOSPC)
	TEST_FAILURE(433, unpack(pkt, 1, "d", &da), ENOSPC)
	TEST_FAILURE(434, unpack(pkt, 2, "d", &da), ENOSPC)
	TEST_FAILURE(435, unpack(pkt, 3, "d", &da), ENOSPC)
	TEST_FAILURE(436, unpack(pkt, 4, "d", &da), ENOSPC)
	TEST_FAILURE(437, unpack(pkt, 5, "d2", &da, &db), ENOSPC)
	TEST_FAILURE(438, unpack(pkt, 6, "d*", 2, &da, &db), ENOSPC)
	TEST_FAILURE(439, unpack(pkt, 10, "d?", &length, 1, &da), EINVAL)

	TEST_FAILURE(440, unpack(pkt, 1, "x0"), EINVAL)
	TEST_FAILURE(441, unpack(pkt, 1, "x*", 0), EINVAL)
	TEST_FAILURE(442, unpack(pkt, 1, "x*", -1), EINVAL)
	TEST_FAILURE(443, unpack(pkt, 0, "x"), ENOSPC)
	TEST_FAILURE(444, unpack(pkt, 1, "x2"), ENOSPC)
	TEST_FAILURE(445, unpack(pkt, 1, "x*", 2), ENOSPC)
	TEST_FAILURE(446, unpack(pkt, 5, "x?", &length, 10, &a), EINVAL)

	TEST_FAILURE(447, unpack(pkt, 1, "X0"), EINVAL)
	TEST_FAILURE(448, unpack(pkt, 1, "X*", 0), EINVAL)
	TEST_FAILURE(449, unpack(pkt, 1, "X*", -1), EINVAL)
	TEST_FAILURE(450, unpack(pkt, 0, "X"), EINVAL)
	TEST_FAILURE(451, unpack(pkt, 1, "X"), EINVAL)
	TEST_FAILURE(452, unpack(pkt, 1, "X2"), EINVAL)
	TEST_FAILURE(453, unpack(pkt, 1024, "X*", 2), EINVAL)
	TEST_FAILURE(454, unpack(pkt, 1024, "X?", &length, 10, &a), EINVAL)

	TEST_FAILURE(455, unpack(pkt, 1, "@0"), EINVAL)
	TEST_FAILURE(456, unpack(pkt, 1, "@*", 0), EINVAL)
	TEST_FAILURE(457, unpack(pkt, 1, "@*", -1), EINVAL)
	TEST_FAILURE(458, unpack(pkt, 0, "@"), ENOSPC)
	TEST_FAILURE(459, unpack(pkt, 0, "@1"), ENOSPC)
	TEST_FAILURE(460, unpack(pkt, 1, "@2"), ENOSPC)
	TEST_FAILURE(461, unpack(pkt, 1, "@*", 2), ENOSPC)
	TEST_FAILURE(462, unpack(pkt, 1, "@?", &length, 10, a), EINVAL)

	TEST_FAILURE(463, unpack(pkt, 1, "?", 0), EINVAL)
...
#define TEST_TRUNC(i, fmt, size, pfmt, len, data1, data2, init) \
	...
	TEST_TRUNC(464, "a3", 3, "%3.3s", 3, a, tstmem, '\0')
	TEST_TRUNC(465, "z3", 3, "%s", 3, z, tstmem, ' ')
	TEST_TRUNC(466, "b3", 1, "%s", 3, b, tstmem, ' ')
	TEST_TRUNC(467, "h3", 2, "%s", 3, h, tstmem, ' ')

	/* Test net_expect() and net_send() */

	server = net_server("localhost", NULL, 30001, 0, 0, NULL, NULL);
	if (server == -1)
		++errors, printf("Test468: net_server(\"localhost\", 30001) failed: %s\n", strerror(errno));
	else
	{
		switch (pid = fork())
		{
			...
			default:
			{
				...
				if (read_timeout(server, 5, 0) == -1 || (s = accept(server, (sockaddr_t *)addr, &addrlen)) == -1)
					++errors, printf("Test469: accept() failed (%s)\n", strerror(errno));
				else
				{
					char test[4];
					ssize_t rc;
					...
					if ((rc = net_expect(s, 5, "%s", test)) != 1)
						++errors, printf("Test470: net_expect(s, 5, HELO) failed (%s)\n", (rc == 0) ? "eof" : strerror(errno));
					else if (memcmp(test, "HELO", 4))
						++errors, printf("Test471: net_expect(s, 5, HELO) failed (received \"%4.4s\", not \"%4.4s\")\n", test, "HELO");
					else if (net_send(s, 5, "OLEH") == -1)
						++errors, printf("Test472: net_send(s, 5, OLEH) failed (%s)\n", strerror(errno));
					if (close(s) == -1)
						++errors, printf("Test473: close(s) failed (%s)\n", strerror(errno));
				}
			...
			case 0:
			{
				...
				for (i = 0; i < 5; ++i)
				{
					c = net_client("localhost", NULL, 30001, 5, 0, 0, NULL, NULL);
					if (c != -1)
						break;
					sleep(1);
				}
				...
				if (c == -1)
					++errors, printf("Test474: net_client(\"localhost\", 30001) failed (%s)\n", strerror(errno));
				else
				{
					char test[4];
					ssize_t rc;

					if (net_send(c, 5, "HELO") == -1)
						++errors, printf("Test475: net_send(c, 5, HELO) failed (%s)\n", strerror(errno));
					else if ((rc = net_expect(c, 5, "%s", test)) != 1)
						++errors, printf("Test476: net_expect(c, OLEH) failed (%s)\n", (rc == 0) ? "eof" : strerror(errno));
					else if (memcmp(test, "OLEH", 4))
						++errors, printf("Test477: net_expect(c, OLEH) failed (received \"%4.4s\", not \"%4.4s\")\n", test, "OLEH");
					if (close(c) == -1)
						++errors, printf("Test478: close(c) failed (%s)\n", strerror(errno));
				}
		...
		if (close(server) == -1)
			++errors, printf("Test479: close(server) failed (%s)\n", strerror(errno));
	}

	/* Test net_pack(), net_packto(), net_unpack() and net_unpackfrom()*/

	server = net_udp_server("localhost", NULL, 30001, 0, 0, NULL, NULL);
	if (server == -1)
		++errors, printf("Test480: net_udp_server(\"localhost\", 30001) failed: %s\n", strerror(errno));
	else
	{
		switch (pid = fork())
		{
			...
			default:
			{
				...
				if (read_timeout(server, 5, 0) == -1 || net_unpackfrom(server, 0, (sockaddr_t *)&ipaddr, &ipsize, "iz4", &neti, netz) == -1)
					++errors, printf("Test481: net_unpackfrom(server, \"iz4\", 37, HELO) failed (%s)\n", strerror(errno));
				else if (neti != 37)
					++errors, printf("Test482: net_unpackfrom(server, \"iz4\", 37, HELO) failed (%d != %d)\n", neti, 37);
				else if (strcmp(netz, "HELO"))
					++errors, printf("Test483: net_unpackfrom(server, \"iz4\", 37, HELO) failed (recv \"%4.4s\", not \"%4.4s\")\n", netz, "HELO");
				else if (write_timeout(server, 5, 0) == -1 || net_packto(server, 0, (sockaddr_t *)&ipaddr, ipsize, "iz4", neti + 1, "OLEH") == -1)
					++errors, printf("Test484: net_packto(server, \"iz4\", 38, OLEH) failed (%s)\n", strerror(errno));
			...
			case 0:
			{
				...
				for (i = 0; i < 5; ++i)
				{
					c = net_udp_client("localhost", NULL, 30001, 0, 0, NULL, NULL);
					if (c != -1)
						break;
					sleep(1);
				}
				...
				if (c == -1)
					++errors, printf("Test485: net_udp_client(\"localhost\", 30001) failed (%s)\n", strerror(errno));
				else
				{
					int neti;
					char netz[4];

					if (write_timeout(c, 5, 0) == -1 || net_pack(c, 0, "iz4", 37, "HELO") == -1)
						++errors, printf("Test486: net_pack(c, \"iz4\", 37, HELO) failed (%s)\n", strerror(errno));
					else if (read_timeout(c, 5, 0) == -1 || net_unpack(c, 0, "iz4", &neti, netz) == -1)
						++errors, printf("Test487: net_unpack(c, 38, OLEH) failed (%s)\n", strerror(errno));
					else if (neti != 38)
						++errors, printf("Test488: net_unpack(c, 38, OLEH) failed (%d != %d)\n", neti, 38);
					else if (strcmp(netz, "OLEH"))
						++errors, printf("Test489: net_unpack(c, 38, OLEH) failed (recv \"%4.4s\", not \"%4.4s\")\n", netz, "OLEH");
					if (close(c) == -1)
						++errors, printf("Test490: close(c) failed (%s)\n", strerror(errno));
				}
		...
		if (close(server) == -1)
			++errors, printf("Test491: close(server) failed (%s)\n", strerror(errno));
	}
	...
	pwd = getpwuid(getuid());
	if (!pwd)
		++errors, printf("Test492: failed to test mail() (getpwuid() failed))\n");
	else
	{
		...
		if (uname(utsbuf) == -1)
			++errors, printf("Test492: failed to test mail() (uname() failed %s)\n", strerror(errno));
		else
		{
			snprintf(addr, 128, "%s@%s", pwd->pw_name, utsbuf->nodename);
			rc = mail(NULL, addr, addr, "subject", "message");
			if (rc != 0)
				++errors, printf("Test492: mail(NULL, \"%s\", \"%s\", \"%s\", \"%s\") failed: returned %d, not %d (%s)\n", addr, addr, "subject", "message", rc, 0, strerror(errno));
		}

		TEST_FAILURE(493, mail(NULL, addr, NULL, NULL, NULL), EINVAL)
		TEST_FAILURE(494, mail(NULL, NULL, addr, NULL, NULL), EINVAL)
	}

	if (errors)
		printf("%d/494 tests failed\n", errors);
	else
